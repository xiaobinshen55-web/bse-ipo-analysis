...
(function() {
  ...
  function processData(rawData) {
    var now = new Date();
    var cutoff = new Date(now.getTime() - 185 * 24 * 60 * 60 * 1000);
    var cutoffStr = cutoff.toISOString().split('T')[0];

    var filtered = rawData.filter(function(d) {
      return d.apply_date && d.apply_date >= cutoffStr && String(d.code).startsWith('920');
    });
  ...
  function tryFetchAPI(callback) {
    var apiUrl = 'https://datacenter.eastmoney.com/api/data/v1/get' +
